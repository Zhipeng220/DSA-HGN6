...
class DifferentiableSparseHypergraph(nn.Module):
    ...
    def __init__(self, in_channels, num_hyperedges, ratio=2, use_virtual_conn=True, **kwargs):
        """
        Args:
            ratio: [FIX Suggestion 2] Changed default from 8 to 2.
                   Ensures inter_channels (e.g., 32) > num_hyperedges (16) to allow orthogonality.
        """
        ...
        # [FIX] ratio=2 -> inter_channels=32 (for base_channel=64)
        ...
        # 确保 inter_channels >= num_hyperedges 以支持正交性
        if inter_channels < num_hyperedges:
            print(f"[Warning] inter_channels ({inter_channels}) < num_hyperedges ({num_hyperedges}). "
                  f"Orthogonality loss will be stuck. Consider lowering 'ratio'.")
...
class PhysicallyInformedHypergraph(nn.Module):
    ...
    def __init__(self, in_channels, num_hyperedges, num_joints=None,
                 num_static_edges=5, init_alpha=0.5, ratio=2, # [FIX Suggestion 2] Changed default from 8 to 2
        ...
        Args:
            ...
            ratio: [FIX] Default 2 to prevent bottleneck
        ...
        print(f"[PhysicalHypergraph] Static edges: {num_static_edges}, "
              f"Total edges: {num_hyperedges}, Per-edge fusion: Enabled, Ratio: {ratio}")
