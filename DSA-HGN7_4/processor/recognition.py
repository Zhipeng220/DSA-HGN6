...
class FT_Processor(Processor):
    """
    V7.2 Processor: Nested Optimization (Muon + Lookahead)
    Modifications: Removed Label Smoothing and explicit Orthogonality/Entropy losses.
    """
    ...
    def load_model(self):
        ...
        # [MODIFIED V7.2] Removed Label Smoothing.
        # KD provides soft labels, and Muon handles orthogonality internally.
    ...
    def load_optimizer(self):
        """
        Setup Muon + AdamW, then wrap with Lookahead
        """
        if self.arg.optimizer == 'Muon':
            ...
            self.optimizer_adamw = optim.AdamW(
                adamw_params,
                lr=0.002,  # Safe fixed LR for Bias/BN
    ...
    def train(self, epoch):
        ...
        for (data_bone, label), batch_joint in iterator:
            ...
            # [MODIFIED V7.2] Disable Entropy and Orthogonality Loss
            # To avoid conflict with Muon optimizer's internal orthogonalization
            loss_entropy = torch.tensor(0.0).to(self.dev)
            loss_ortho = torch.tensor(0.0).to(self.dev)

            # Commented out the loop to save compute:
            ...
            # Force regularization weights to 0.0
            lambda_ent = 0.0
            lambda_orth = 0.0
    ...
    def adjust_lr(self):
        ...
        if hasattr(self.arg, 'warm_up_epoch') and self.arg.warm_up_epoch > 0:
            if current_epoch < self.arg.warm_up_epoch:
                # 保持原有逻辑: (epoch + 1) / warm_up
                # 例如 epoch=1, warm_up=5 -> factor = 0.4
                factor = (current_epoch + 1) / self.arg.warm_up_epoch
            else:
                # Warm up 结束后进入 Step Decay
                if self.arg.step:
                    # 计算经历了多少个衰减 step
                    factor = lr_decay_rate ** np.sum(current_epoch >= np.array(self.arg.step))

        # 如果没有 Warm up 但有 Step Decay
        elif self.arg.step:
            factor = lr_decay_rate ** np.sum(current_epoch >= np.array(self.arg.step))

        # ---------------------------------------------------------------------
        # 2. 更新优化器组 (Fix: 独立缩放每个参数组)
        # ---------------------------------------------------------------------
        for param_group in self.optimizer.param_groups:
            # 首次运行时，记录该组的初始学习率 (initial_lr)
            # Muon 组会记录 ~0.02，AdamW 组会记录 ~0.002
        ...
        # 3. 更新日志显示的 lr
        # ---------------------------------------------------------------------
        # 仅用于日志打印，显示 Base LR 的变化趋势
        self.lr = self.arg.base_lr * factor
