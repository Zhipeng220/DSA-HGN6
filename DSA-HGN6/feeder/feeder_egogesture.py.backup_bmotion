...
class Feeder(Dataset):
    ...
    def get_mean_map(self):
        ...
        # [FIX 1] 互斥检查(防止同时开启bone和vel)
        if self.bone and self.vel:
            raise ValueError("Cannot compute stats for both bone and velocity simultaneously")

        # [FIX 2] 如果是Bone模式,先将数据转换为Bone向量再计算统计量
        if self.bone:
            print("[Feeder] Calculating mean/std for Bone stream...")
            bone_data = np.zeros_like(data)
            for v1, v2 in self.get_bone_connections():
                bone_data[:, :, :, v1, :] = data[:, :, :, v1, :] - data[:, :, :, v2, :]
            data = bone_data

        # [FIX 3] 如果是Velocity模式,先计算速度再算均值
